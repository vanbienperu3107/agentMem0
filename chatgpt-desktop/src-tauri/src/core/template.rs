...
impl Template {
    ...
    pub fn new<P: AsRef<Path>>(template_dir: P) -> Self {
        ...
        let files: Vec<(std::path::PathBuf, &[u8])> = vec![
            (template_dir.join("ask.js"), SCRIPT_ASK),
            (template_dir.join("chat-logger.js"), SCRIPT_CHAT_LOGGER),
        ];

        for (filename, data) in files {
            match update_or_create_file(&filename, data) {
                Ok(updated) => {
                    if updated {
...
impl Default for Template {
    fn default() -> Template {
        Template {
            ...
            chat_logger: Vec::from(SCRIPT_CHAT_LOGGER),
        }
    }
}
